...
def test_build_nn(build_nn):
    with tf.Graph().as_default():
        ...
        # Check Shape
        print('test_input_data_shape + [test_vocab_size]',test_input_data_shape + [test_vocab_size])
        print('[test_rnn_layer_size, 2, None, test_rnn_size]',[test_rnn_layer_size, 2, None, test_rnn_size])
        assert logits.get_shape().as_list() == test_input_data_shape + [test_vocab_size], \
            'Outputs has wrong shape.  Found shape {} rather than {}'.format(logits.get_shape()).format(test_input_data_shape+test_vocab_size)
        assert final_state.get_shape().as_list() == [test_rnn_layer_size, 2, None, test_rnn_size], \
            'Final state wrong shape.  Found shape {} rather than{}'.format(final_state.get_shape()).format(logits.get_shape()).format(test_input_data_shape+test_vocab_size)
